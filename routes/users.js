...
router.post(
  ...
  upload.any(),
  [
    check("email").isEmail().withMessage("El mail ingresado no es valido"),
    check("name")
      .isLength({ min: 2 })
      .withMessage("El nombre introducido es demasiado breve"),
    check("lastname")
      .isLength({ min: 2 })
      .withMessage("El apellido introducido es demasiado breve"),
    check("date").isLength().withMessage("Debe ingresar una fecha"),
    check("pass")
      .isLength({ min: 8 })
      .withMessage("La contraseña es demasiado breve"),
    body("email")
